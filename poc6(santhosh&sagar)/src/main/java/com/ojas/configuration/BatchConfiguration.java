...
public class BatchConfiguration {
	...
	@Bean
	public Step step1() {
		return stepBuilderFactory.get("step1").<Employee, Employee>chunk(1).reader(reader()).writer(writer())
	...
	public StaxEventItemWriter<Employee> writer() {
		...
		writer.setMarshaller(employeeUnmarshaller());
		writer.setRootTagName("employee");
		return writer;
	}

	@Bean
	public XStreamMarshaller employeeUnmarshaller() {
	...
	public JdbcBatchItemWriter<Employee> writerToDb() {
		...
		writer.setSql("INSERT INTO Employee_Batch(id,name,city) VALUES (:id,:name,:city)");
